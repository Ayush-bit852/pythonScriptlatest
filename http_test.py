    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        try:
            for msg in messages:
                ...
                if port not in socket_map and port not in pending_ports:
                    # Submit connection task to threadpool
                ...
                for f in completed:
                    port_, socket_obj = f.result()
                    if socket_obj:
                        socket_map[port_] = socket_obj
                    pending_ports.discard(port_)
                    futures.pop(f)

                if port in socket_map:
                    try:
